...
### Q: ESP8266 無法啟動或一直重啟
1. **檢查 GPIO0 (D3)**: 若感測器連接至 GPIO0，可能會干擾啟動模式。ESP8266 啟動時 GPIO0 需保持 HIGH，否則會進入燒錄模式。建議改用 D7 (GPIO13) 等安全腳位。
2. **檢查 GPIO15 (D8)**: 此腳位需在啟動時保持拉低 (LOW)，避免連接有預設 HIGH 輸出的模組。
3. **檢查 GPIO2 (D4)**: 啟動時需保持 HIGH，若連接了會拉低的模組可能導致無法開機。
